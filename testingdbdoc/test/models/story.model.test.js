...
describe('Model User createStory', () => {
    ...
    it('Can create story', async () => {
        await Story.createStory('abc', userId);
    ...
    it('Cannot create story with invalid userId', async () => {
        const error = await Story.createStory('abc', '123').catch(error => error);
    ...
    it('Cannot create story with removed userId', async () => {
        ...
        const error = await Story.createStory('abc', userId).catch(error => error);
...
describe('Model User updateStory', () => {
    ...
    beforeEach('Create user for test', async () => {
        ...
        const user2 = await User.signUp('Ti', 'ti@gmail.com', '123');
        userId1 = user1._id;
        userId2 = user2._id;

        const story = await Story.createStory('abc', userId1);
        idStory = story._id;
    });
    ...
    it('Cannot update story with other\'s userId', async () => {
        const error = await Story.updateStory(idStory, userId2, 'xyz').catch(error => error);
        assert.equal(error.code, 'CANNOT_FIND_STORY');
        const story1 = await Story.findOne({});
        assert.equal(story1.content, 'abc');
    });

    it('Cannot update story without content', async () => {
        const error = await Story.updateStory(idStory, userId1, '').catch(error => error);
    ...
    it('Cannot update story with invalid userId', async () => {
        const error = await Story.updateStory(idStory, 'abcd', '').catch(error => error);
        assert.equal(error.code, 'INVALID_ID');
        const story1 = await Story.findOne({});
        assert.equal(story1.content, 'abc');
    });
    ...
    it('Cannot update story with invalid idStory', async () => {
        const error = await Story.updateStory('abcd', userId1, 'abcd').catch(error => error);
        assert.equal(error.code, 'INVALID_ID');
        const story1 = await Story.findOne({});
        assert.equal(story1.content, 'abc');
    });
    ...
    it('Cannot update removed story', async () => {
        ...
        const error = await Story.updateStory(idStory, userId1, 'xyz').catch(error => error);
        assert.equal(error.code, 'CANNOT_FIND_STORY');
        const story1 = await Story.findOne({});
...
describe('Model User removeStory', () => {
    ...
    it('Cannot remove story with other\'s userId', async () => {
        const error = await Story.removeStory(idStory, userId2).catch(error => error);
        assert.equal(error.code, 'CANNOT_FIND_STORY');
    ...
    it('Cannot remove story with invalid userId', async () => {
        const error = await Story.removeStory(idStory, 'abc').catch(error => error);
        assert.equal(error.code, 'INVALID_ID');
        const story = await Story.findById(idStory);
        assert.equal(story.content, 'abc');
    });
    ...
    it('Cannot remove story with invalid idStory', async () => {
        const error = await Story.removeStory('ax', userId1).catch(error => error);
        assert.equal(error.code, 'INVALID_ID');
        const story = await Story.findById(idStory);
        assert.equal(story.content, 'abc');
    });
    ...
    it('Cannot remove a removed story', async () => {
        await Story.findByIdAndRemove(idStory);
        const error = await Story.removeStory(idStory, userId1).catch(error => error);
        assert.equal(error.code, 'CANNOT_FIND_STORY');
        const story = await Story.findById(idStory);
        assert.equal(story,null);
    });
